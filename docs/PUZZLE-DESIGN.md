...
beat + one zh beat on _this_ engine → **derived engine requirements**.
...
- zh beat: 日记写"她生日就是网吧开业那天"；IE 收藏夹里网吧主页写着"隆重开业
  2004年3月18日"→ 密码 0318。
- **Derived**: no new events — this mechanic is pure content. It _demands_
...
The player proves understanding by _filling a constrained answer form_ — Mad
...
- zh beat: 结局的"给班主任的信"填空：事发那晚，** 在 ** 看见了 \_\_ ——
...
- **Derived**: no new events — this is a _pattern over existing machinery_:
...
- Genre: _Emily is Away_ (mash-keys typing, predetermined replies — the input
...
the _skin_: QQ vs MSN, hao123 vs AltaVista-alike, 网吧 receipts vs summer-camp
...
`evidence`, `deduction` — all three are _scenario-layer surfaces_, so their
...
The owner's question — "how does all of this become _convenient to orchestrate
in code_?" — resolves into four authoring layers that compile to one runtime:
...
s.on('file:open', { path: 'D:/日记.txt' })
  .when(not(flag('act2')))
  .once()
  .do(collect('term.bicycle'), delay('90s', msg.send('阿哲', script('nudge-1'))));
...
`rehearsal: true` marker the host `onEvent` bridge drops — so fast-forwarding
