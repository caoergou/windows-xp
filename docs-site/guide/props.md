...
| `scenario` | `Scenario` | — | Declarative scenario/story script: flags, triggers, and gated actions authored as plain JSON. See [`docs/SCENARIOS.md`](https://github.com/caoergou/windows-xp/blob/main/docs/SCENARIOS.md) |
| `lessons` | `Lesson[]` | — | Guided lessons: data-driven Watch/Try/Do tutorials. Register them here, then start one via the `ref` handle. See [`docs/LESSONS.md`](https://github.com/caoergou/windows-xp/blob/main/docs/LESSONS.md) |
| `onEvent` | `(e: XPEvent) => void` | — | Subscribe to every desktop event (see [Events](/guide/events)) |
| `devtools` | boolean | `false` | Mount the Scenario / event DevTools overlay (dev-time; tree-shaken out when off). See [`docs/SCENARIOS.md`](https://github.com/caoergou/windows-xp/blob/main/docs/SCENARIOS.md) |
| `mode` | `'fullscreen'` \| `'embedded'` | `'fullscreen'` | `'embedded'` disables all host-page hijacking in one switch |
| `viewportPolicy` | `'auto'` \| `'scale'` \| `'native'` \| `'warn'` | `'auto'` (fullscreen) / `'native'` (embedded) | Small-screen strategy — see [Small screens & mobile](/guide/embedding#small-screens-mobile) |
