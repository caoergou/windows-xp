...
| BOOT-05 | "关闭计算机"对话框：待机（黄）/ 关闭（红）/ 重新启动（绿）三按钮；**弹出时桌面其余部分渐变为灰度** | ⭐⭐⭐ |  ✅  | 已按 XP 截图重做 314×200 三段式蓝色面板、32px 黄/红/绿按钮与灰度桌面，并由 `e2e/fidelity87.spec.ts` 锁定结构/色值/尺寸；关机与重启现进入可配置的请求→黑屏→完成生命周期（#279），待机保留不可操作语义 |
...
| DSK-07 | 桌面右键菜单：排列图标 / 刷新 / 粘贴 / 新建（文件夹、快捷方式、文本文档）/ 属性 | ⭐⭐⭐ |  🟡  | `ContextMenu` + `DesktopProperties` 已有；"显示属性"对话框标签栏/按钮统一为共享 `XPTabs`/`XPButton`，补齐真实 XP 的 5 个标签页（主题/桌面/屏幕保护程序/外观/设置，仅"桌面"页可换壁纸，其余占位）；菜单项完整度待比对 |
...
| APP-03 | QQ 消息记录管理器：好友／群会话、全文搜索、时间线与附件入口 |  ⭐⭐  |  ✅  | #280：按 QQ 2006／早期消息管理器重做蓝灰工具栏、联系人树、日期分隔的无气泡记录区和底部分页；复用 QQ 头像／按钮资产；内容包声明只读档案，查询状态按实例持久化并发出稳定档案事件，并由 Playwright 覆盖入口、记录搜索与附件展示。 |
| APP-04 | IE 时代网页模板与离线搜索语料：门户／新闻／论坛／个人主页 |  ⭐⭐  |  ✅  | #281：结构化页面经沙箱 iframe 渲染；搜索支持 Unicode 归一化、别名、稳定排序和按条目声明的轻量拼写容错，同时兼容原有 HTML 路径                                                                           |
| APP-05 | 扫雷切换难度（初级／中级／高级）后窗口按新棋盘尺寸自适应收放 |  ⭐⭐  |  ✅  | 难度切换在同一渲染周期内同步重建棋盘（render-time state adjustment），useLayoutEffect 始终量到与新 config 一致的棋盘；修复中级↔高级（同为 16 行）board.length 不变、不再触发重测导致窗口永久停在错误尺寸的问题 |
| APP-06 | 纸牌（Solitaire）完整规则：Draw One/Three、标准计分、计时、一步撤销、右键收牌、胜利弹跳动画 |  ⭐⭐  |  ✅  | sol.exe 行为：菜单切换翻 1/3 张并开新局（当前项打勾）；标准计分（waste→tableau +5、→foundation +10、tableau→foundation +10、翻暗牌 +5、foundation→tableau -15，Draw1 每次重过 stock -100、Draw3 第三次起 -20，分数不为负）；状态栏显示分数与时间；Ctrl+Z/菜单撤销最近一步（含分数快照）；右键点牌自动进 foundation、右键拖拽取消；胜利时 foundation 逐张弹出带初速度+重力弹跳（rAF 物理、点击可跳过、期间禁操作）；修复 tableau 取牌索引误用 offsetY 与长牌堆下部不可放置两个拖拽 bug；补"关于纸牌"对话框 |
| APP-07 | 千千静听（TTPlayer）纳入 zh 文化应用层：主窗口 LED 屏+频谱+走带控制、可增删的播放列表、歌词秀滚动高亮 |  ⭐⭐  |  ✅  | 绿黑高对比点阵 LED 皮肤全部自绘致敬（布局参考 haixiangyan/ttplayer，MIT；见 src/apps/TTPlayer/NOTICE.md，关于面板含致谢行）；播放复用 Winamp/WMP 同款内置 sample 音频管线（无 new Audio），显示时长/歌词按数据层声明时长等比映射；曲目与 LRC 数据驱动（content.ts，歌词为原创占位文本）；注册 locales zh + 开始菜单 recent，渲染冒烟入 cultureApps 测试 |
...
| ID     | XP 基准                                                                                          | 感知度 | 状态 | 备注                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ------ | ------------------------------------------------------------------------------------------------ | :----: | :--: | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| STY-01 | 窗口 chrome：Luna 蓝渐变标题栏、顶部圆角、粗边框、贴图控制按钮                                   | ⭐⭐⭐ |  ✅  | #35 已审计修正一轮（去现代阴影、Trebuchet MS 标题字体）；#213 将该实现装入 `xpOS.chrome.WindowDecoration`，默认包仍复用原组件与 token，XP 视觉不变；`paperOS` 是原创契约 dogfood，不属于 XP fidelity 基线                                                                                                                                                                                                                                                                                                                                    |
