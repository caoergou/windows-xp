...
describe('validateScenario', () => {
  ...
  it('rejects an unknown action key with its path', () => {
    const r = validateScenario({
      id: 's',
      triggers: [{ on: 'cmd:exec', do: [{ setFlags: 'oops' }] }],
    });
    expect(r.ok).toBe(false);
  ...
  it('rejects an unknown condition key', () => {
    const r = validateScenario({
      id: 's',
      triggers: [{ on: 'cmd:exec', when: { flagg: 'x' }, do: [{ setFlag: 'y' }] }],
    });
  ...
  it('warns on an unknown event domain (still parses)', () => {
    const r = validateScenario({
      id: 's',
      triggers: [{ on: 'bogus:thing', do: [{ setFlag: 'y' }] }],
    });
    expect(r.errors).toEqual([]);
  ...
  it('warns on a flag read but never set', () => {
    const r = validateScenario({
      id: 's',
      triggers: [{ on: 'cmd:exec', when: { flag: 'ghost' }, do: [{ setFlag: 'y' }] }],
    });
    expect(r.warnings.join('\n')).toContain('"ghost"');
    expect(r.warnings.join('\n')).toContain('never set');
  });
  ...
  it('checks note action shapes (#207)', () => {
    const missingContent = validateScenario({
      id: 's',
      triggers: [{ on: 'cmd:exec', do: [{ note: { id: 'x' } }] }],
    });
    expect(missingContent.errors.join('\n')).toMatch(/triggers\[0\]\.do\[0\]\.note.*content/);
    const badRemove = validateScenario({
      id: 's',
      triggers: [{ on: 'cmd:exec', do: [{ removeNote: 123 }] }],
    });
    expect(badRemove.errors.join('\n')).toContain('triggers[0].do[0].removeNote');
    const good = validateScenario({
      id: 's',
      triggers: [
        { on: 'cmd:exec', do: [{ note: { id: 'x', content: 'hi' } }, { removeNote: 'x' }] },
      ],
    });
  ...
  it('rejects an oversized scenario', () => {
    const big = {
      id: 's',
      triggers: [{ on: 'cmd:exec', do: [{ setFlag: 'x'.repeat(SCENARIO_MAX_BYTES + 1) }] }],
    };
