...
## WindowFactory restoration (`src/utils/WindowFactory.tsx`)

The persisted window list (`<prefix>open_windows`) is **versioned** via
`src/utils/windowPersistence.ts` (`encodeOpenWindows`/`decodeOpenWindows`,
`OPEN_WINDOWS_VERSION`). A load only accepts the current version; the pre-#163
bare-array format (or a newer version) is discarded, so every entry that reaches
`restoreComponent` is guaranteed to carry a real registry `appId`. Restoration is
therefore just two cases (the pre-registry prop-sniffing heuristics and
id/中文名 aliases were retired in #163 C):

1. **Exact appId match** in `APP_REGISTRY`
2. **Dynamic `properties-*` appId** → `FileProperties`

| Props / appId | Restored as |
|---|---|
| appId starts with 'properties-' | FileProperties |
| Any appId in APP_REGISTRY | Exact match restoration |
| anything else | dropped (logged) |
