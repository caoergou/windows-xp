/**
 * Windows XP icon registry (#213) — the name → URL binding for every icon the
 * desktop uses, owned by the theme package so the engine/components resolve
 * icons purely by name. A future theme supplies its own map of the same shape
 * (system icons are theme chrome; app/brand icons ride along here until apps
 * contribute their own icons via the registry — #128/#143 follow-up).
 *
 * Authentic Windows XP system/app icons from xp-research/organized/icons/XPIcons/XP/
 * and react-xp/frontend/public/, resized to a consistent 48x48. Imported as
 * modules so Vite handles the base URL and hashing.
 */
...
export const XP_ICONS: Record<string, string> = {
