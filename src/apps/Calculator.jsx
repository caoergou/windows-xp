...
const Calculator = ({ windowId }) => {
  ...
  const [operand, setOperand]     = useState(null);   // 等待计算的第一个操作数
  const [operator, setOperator]   = useState(null);   // 当前运算符
  const [waitNext, setWaitNext]   = useState(false);  // 下次按数字是否新开
  ...
  const formatResult = (n) => {
    if (n === 'Infinity' || !isFinite(n)) return 'Infinity';
    const s = parseFloat(n.toPrecision(10)).toString();
    return s.length > MAX_DIGITS ? parseFloat(n.toPrecision(8)).toString() : s;
  };
